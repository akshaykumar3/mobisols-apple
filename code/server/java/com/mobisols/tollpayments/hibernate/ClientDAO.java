 ...
 * 
 * @see com.mobisols.tollpayments.hibernate.Client
...
public class ClientDAO extends HibernateDaoSupport {
	...
	public Client findById(com.mobisols.tollpayments.hibernate.ClientId id) {
		log.debug("getting Client instance with id: " + id);
		try {
			Client instance = (Client) getHibernateTemplate().get(
					"com.mobisols.tollpayments.hibernate.Client", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Client> findByExample(Client instance) {
		...
		try {
			List<Client> results = (List<Client>) getHibernateTemplate()
