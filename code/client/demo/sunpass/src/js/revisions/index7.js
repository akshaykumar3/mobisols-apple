Ext.setup({
	...
	onReady: function() {
		...
		Ext.Ajax.request({
	      url: 'http://localhost:6004/com.mobisols.tollpayments.mockwebservices/services/AccountDetails',
	      success: function(result) {
	      	...
	      	var vehicle_details=res.response.vehicleDetails[0];
	      	var paidtoll_details=res.response.tollPayments[0];
	      	if(pay_details)
	      	{
			...
			items: [{
				...
				listeners: {
					maprender: function(comp, map) {
						...
						console.log('longiitude: '+geo.longitude);
						...
						mylocation_marker=new google.maps.Marker({
							...
							icon: '../../resources/images/blue_dot.jpg',
							map: map
						});
					...
					{
						console.log('center change event is fired');
						mylocation_marker.setPosition(geo.latitude,geo.longitude);
					}
				}
			},{
				id: 'addcar',
